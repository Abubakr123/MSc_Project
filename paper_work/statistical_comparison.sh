...
echo $DMunc "=> median DM uncertainty" #DMunc

DMchi=`bash ~/MSc_Project/paper_work/scripts/jd_calc_stats.bash -col=5 -median $DMfile`
echo $DMchi "=> median red. chi^2 of the DM fits"

ToAsfit=`bash ~/MSc_Project/paper_work/scripts/jd_calc_stats.bash -col=6 -median $DMfile`
echo $ToAsfit "=> median number of ToAs in the DM fits"

# Weighted RMS of the Delta DMs
...
echo -e "\nThe weighted RMS of the Delta DMs\n"$rms
